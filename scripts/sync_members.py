"""
EloBoard 티어 API를 조회해 StarUniv와 공유하는 Supabase tier_members를 보조 동기화한다.
신규 후보는 tier_member_candidates에 넣고 자동 승인하지 않는다.
"""

import sys
import argparse

from _common import fetch_json, kst_now
...
# "DB에 있던 사람 전원이 API에 없다 -> 전원 신규 후보"가 되거나 반대로
...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        help=f"신규 후보가 {MAX_NEW_CANDIDATES_PER_RUN}명을 넘어도 진행합니다 "
             f"(빈 DB에서 처음 채울 때처럼, 대량 신규가 정상인 경우에만 사용)",
    )
    args = parser.parse_args(argv)

    if not is_roster_ready():
        print("[오류] Supabase 로스터 자격 증명이 설정되지 않았습니다.", file=sys.stderr)
        sys.exit(1)

    member_map = load_roster_members()
    if member_map is None:
        print("[오류] tier_members를 읽지 못해 동기화를 중단합니다 "
              "(읽기 실패를 '기존 회원 0명'으로 착각해 전원을 신규로 "
              "오판하는 사고를 막기 위함 - SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY/권한을 확인하세요).",
              file=sys.stderr)
        sys.exit(1)
    # soop_id 대소문자/공백 차이 때문에 이미 있는 사람을 "신규"로 오판해서
    # 매 실행마다 같은 사람이 중복으로 계속 추가되는 걸 막기 위해, 소문자로
    # 정규화한 키로도 조회할 수 있게 별도 맵을 만든다. DB에 실제로 저장된
    # 원래 대소문자는 그대로 보존해야 하므로(정규화된 키로 덮어쓰면 안 됨),
    # 조회는 정규화된 키로 하되 갱신은 항상 DB에 있던 원래 soop_id를 쓴다.
    normalized_to_original = {sid.strip().lower(): sid for sid in member_map}

    # tier_members에 없는 신규 등록자는 이제 곧바로 tier_members에 쓰지
    # 않고 new_members(대기) 시트로 보낸다 - 관리자가 검토해서 직접
    # tier_members로 옮기기 전까지는 여기 머무른다. 이미 대기 중인 후보를
    # 정규화된 soop_id 기준으로 미리 알아둬야, 아직 검토 안 끝난 같은
    # 후보가 매 실행마다 new_tier_members에 중복으로 또 쌓이지 않는다.
    pending_map = load_pending_members()
    if pending_map is None:
        # new_tier_members 읽기 실패는 tier_members 읽기 실패보다는 훨씬
        # 가볍다 - 최악의 경우 대기 DB에 중복 후보 행이 하나 더 생기는
        # 정도라 동기화 자체를 중단할 이유는 없다. 다만 조용히 넘어가면
        # 원인 파악이 어려우니 경고는 남긴다.
        print(f"[경고] '{PENDING_TABLE_NAME}' DB를 읽지 못해 대기 중 후보 중복 체크를 건너뜁니다.", file=sys.stderr)
        pending_map = {}
    ...
    if not api_players:
        ...
        print("[오류] 티어 API에서 선수를 한 명도 얻지 못했습니다 - DB를 건드리지 않고 중단합니다.",
              file=sys.stderr)
        sys.exit(1)
    ...
    # new_tier_members에 중복 행으로 추가되어버린다. 이번 실행에서 이미
    ...
    for api_player in api_players:
        ...
        # API가 주는 soop_id의 대소문자/공백이 DB에 저장된 것과 다를 수 있어서,
        # 정규화된 키로 먼저 조회하고, 찾았으면 DB에 있던 원래 키를 그대로 쓴다
        # (updates 딕셔너리의 키가 update_roster_members()에서 실제 행을 찾는 기준이라,
        ...
        # 오판돼 new_tier_members에 중복으로 계속 쌓인다.
        ...
        if is_existing_member:
            ...
            if normalized_id in already_pending_normalized or normalized_id in pending_normalized_ids:
                # 이미 new_tier_members에 대기 중이거나(이전 실행에서
    ...
    if args.dry_run:
        ...
        for c in new_candidates:
            print(f"  [신규후보 추가 예정 -> '{PENDING_TABLE_NAME}' 테이블] {c['id']} ({c['nickname']})")
        print(f"[dry-run 완료] 갱신 {len(updates)}명, 신규 후보 {len(new_candidates)}명 (Supabase 로스터는 안 건드림)")
        return

    updates_ok = update_roster_members(updates, []) if updates else True
    ...
    if updates or new_candidates:
        ...
        # 찍혀서, 로그를 보고도 DB에 아무것도 안 들어간 걸 알 수 없었다.
        if updates_ok and candidates_ok:
            print(f"[완료] 기존 정보 갱신 {len(updates)}명, 신규 후보 {len(new_candidates)}명을 "
                  f"'{PENDING_TABLE_NAME}' DB에 추가함 (검토 후 tier_members로 옮겨주세요).")
        else:
            print(f"[경고] Supabase 쓰기 일부 실패 (갱신 {len(updates)}명: "
